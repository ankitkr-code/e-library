...
def fuzzy_match(query, text):
    """
    Fuzzy matching to find similar keywords
    Handles: substring match, word match, partial word match, singular/plural
    """
    ...
    # Check if any query word is contained in text words or vice versa
    for q_word in query_words:
        for t_word in text_words:
            # Word match or prefix match (handles singular/plural)
            if q_word in t_word or t_word in q_word:
                return True
    
    # Check for partial word matches (at least 70% match)
    for q_word in query_words:
        for t_word in text_words:
            # Simple similarity: if 70% of shorter word is in longer word
            if len(q_word) >= 3 and len(t_word) >= 3:
                shorter = min(q_word, t_word)
                longer = max(q_word, t_word)
                match_count = sum(1 for c in shorter if c in longer)
                if match_count >= len(shorter) * 0.7:
                    return True
    
    return False

def search_books(query):
    """Search books by title, author, or category with fuzzy matching"""
    results = []
    for book in books:
        if (fuzzy_match(query, book.get('title', '')) or 
            fuzzy_match(query, book.get('author', '')) or 
            fuzzy_match(query, book.get('category', ''))):
            results.append({
                'type': 'Book',
...
def search_programs(query):
    """Search programs by name, department, or description with fuzzy matching"""
    results = []
    for program in programs:
        if (fuzzy_match(query, program.get('name', '')) or 
            fuzzy_match(query, program.get('department', '')) or 
            fuzzy_match(query, program.get('description', ''))):
            results.append({
                'type': 'Program',
...
def search_notices(query):
    """Search notices by title or content with fuzzy matching"""
    results = []
    for notice in notices:
        if (fuzzy_match(query, notice.get('title', '')) or 
            fuzzy_match(query, notice.get('content', ''))):
            results.append({
                'type': 'Notice',
                'title': notice.get('title', 'N/A'),
                'content': notice.get('content', 'N/A')[:100] + '...',
                'detail': f"Posted on {notice.get('date', 'N/A')}"
            })
    return results

def search_syllabus(query):
    """Search syllabus by subject or content with fuzzy matching"""
    results = []
    for item in syllabus_items:
        if (fuzzy_match(query, item.get('subject', '')) or 
            fuzzy_match(query, item.get('content', ''))):
            results.append({
                'type': 'Syllabus',
                'title': item.get('subject', 'N/A'),
                'department': item.get('department', 'N/A'),
                'detail': f"Department: {item.get('department', 'N/A')}"
            })
    return results

def search_class_notes(query):
    """Search class notes by subject or topic with fuzzy matching"""
    results = []
    for note in class_notes:
        if (fuzzy_match(query, note.get('subject', '')) or 
            fuzzy_match(query, note.get('topic', '')) or 
            fuzzy_match(query, note.get('content', ''))):
            results.append({
                'type': 'Class Notes',
...
def search_academic_tasks(query):
    """Search academic tasks by title or description with fuzzy matching"""
    results = []
    for task in academic_tasks:
        if (fuzzy_match(query, task.get('title', '')) or 
            fuzzy_match(query, task.get('description', '')) or
            fuzzy_match(query, task.get('subject', ''))):
            results.append({
                'type': 'Academic Task',
...
def manage_location():
    ...
    return render_template('manage_location.html', title='Manage Location', subtitle='Add or update library locations', locations=locations)
